  ...
  return (
    <Container>
      ...
      {state.bookmarks.length > 0 ? (
        <>
          {pages > 1 && <PaginationList>{getPaginationItems()}</PaginationList>}
          <BookmarkList>
            {state.bookmarks
              .slice(firstArrayElement, firstArrayElement + pageSize)
              .map((bookmark) => {
                return (
                  <BookmarkItem key={bookmark.id}>
                    <BookmarkLink href={bookmark.url} target="_blank" rel="noreferrer noopener">
                      ...
                      <BookmarkDate>
                        Ajouté le <Date date={bookmark.createdDate} />
                      </BookmarkDate>
                      {bookmark.tags.length > 0 && (
                        <TagsList>
                          {bookmark.tags.map((tag) => (
                            <TagsItem key={tag.value}>{tag.value}</TagsItem>
                          ))}
                        </TagsList>
                      )}
                      ...
                      {bookmark.duration && (
                        <BookmarkInfo>
                          Durée : {formatDuration(bookmark.duration * 1000)}
                        </BookmarkInfo>
                      )}
                    </BookmarkLink>
                    <BookmarkActions>
                      <ButtonLink to={`/edit/${bookmark.id}`}>Modifier</ButtonLink>
                      <Button onClick={() => handleClick(bookmark.id)} secondary>
                        Supprimer
                      </Button>
                    </BookmarkActions>
                  </BookmarkItem>
